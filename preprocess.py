...
def clean_unused_images():
    ...
    for dir_id, img_ids in seq_frame.items():
        ...
        dir_path = os.path.join(par.image_dir, dir_id)
        print("dir path : ", dir_path)
        if not os.path.exists(dir_path):
            print("didnt do anything in {}".format(dir_id))
            continue
        ...
        for idx in range(0, start):
            ...
            img_path = os.path.join(par.image_dir, dir_id, img_name)
...
def create_pose_data():
    info = {'00': [0, 4540], '01': [0, 1100], '02': [0, 4660], '04': [0, 270], '05': [0, 2760],
            '06': [0, 1100], '07': [0, 1100], '08': [1100, 5170], '09': [0, 1590], '10': [0, 1200], '11' : [0, 581]}
    start_t = time.time()
    for video in info.keys():
        ...
        with open(fn) as f:
            ...
            poses = []
            for l in lines:
                print(f"Processing line: {l.strip()}")  # Print the current line
                pose = R_to_angle([float(value) for value in l.strip().split()])  # Process the line
                poses.append(pose)
            ###
            ...
            print('Saved pose data to:', base_fn + '.npy')
